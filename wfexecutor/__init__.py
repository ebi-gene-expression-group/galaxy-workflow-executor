    ...
    for dataset in datasets:
        if dataset['type'] == 'file':
            ...
            if dataset['name'] is not None:
                ...
                uploaded_dataset = gi.libraries.upload_from_galaxy_filesystem(lib_id, file_path, folder_id=folder_id, link_data_only="link_to_files")
...
def load_input_files(gi, inputs, workflow, history):
    ...
    for step, step_data in workflow['inputs'].items():
        ...
        if step_data['label'] in inputs and 'path' in inputs[step_data['label']]:
            ...
            gi.histories.update_dataset(history['id'], upload_res['outputs'][0]['id'], misc_info=dataset_details['misc_info'])
